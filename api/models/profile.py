...
class Profile(models.Model):
    ...
    external_id = models.IntegerField("Код сотрудника",
                                      validators=[RegexValidator(r'^\d{6}$')],
                                      unique=True, default=get_new_external_id)
    ...
    status = models.CharField("Статус", max_length=7, choices=status,
                              default="pure")
    qr = models.TextField("QR-код", blank=True, null=True)
    payment_url = models.CharField("URL страницы оплаты", max_length=45,
                                   blank=True)
    balance = models.DecimalField("Баланс", max_digits=8, decimal_places=2,
                                  default=0.00)
    test_balance = models.DecimalField("Тестовый баланс", max_digits=8,
                                       decimal_places=2, default=0.00)
    are_notifications_enabled = models.BooleanField("Уведомления включены?",
                                                    default=False)
    fcm_devices = models.ManyToManyField(FCMDevice, verbose_name='Устройства',
                                         blank=True)
